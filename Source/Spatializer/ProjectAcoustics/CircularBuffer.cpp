...
CircularBuffer::CircularBuffer(uint32_t bufferSizeInFrames, uint32_t channels)
    ...
    , m_AudioData(AlignedStore::AlignedAlloc<float>(bufferSizeInFrames * channels))
    , m_ReadPos(0)
    , m_WritePos(0)
    , m_BufferedSamples(0)
{
...
_Use_decl_annotations_ void CircularBuffer::ReadSamples(float* destinationBuffer, uint32_t samplesToRead)
{
...
_Use_decl_annotations_ void CircularBuffer::WriteSamples(float* sourceBuffer, uint32_t samplesToWrite)
{
    this->WriteSamples(sourceBuffer, samplesToWrite, 0u);
}

_Use_decl_annotations_ void CircularBuffer::WriteSamples(float* sourceBuffer, uint32_t samplesToWrite, uint32_t stride)
{
    // Figure out if we're about to buffer overrun
    uint32_t bufferedSamples = m_BufferedSamples;
    ...
    if (m_WritePos + samplesToWrite < m_BufferSize)
    {
        ...
        else
        {
            ...
            {
                m_AudioData[m_WritePos + sample] = sourceBuffer[sample * stride];
            }
        }
