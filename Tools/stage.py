...
def copy_spatializer(platform, source_dir, target_dir):
    if (platform == "Windows" or platform == "WindowsStore"):
        copy_file(oshelpers.fixpath(source_dir, constants.spatializer_name + ".dll"), target_dir)
        copy_file(oshelpers.fixpath(source_dir, constants.spatializer_name + ".pdb"), target_dir)
    else:
        copy_file(oshelpers.fixpath(source_dir, "lib" + constants.spatializer_name + ".so"), target_dir)
...
def stage_binaries(artifacts_path = ""):
    git_root = oshelpers.fixpath(githelpers.get_root())
    plugin_path_under_project = oshelpers.fixpath(constants.unity_project_dir, "Assets", constants.spatializer_plugin_name, "Plugins")
    ...
    if not artifacts_path:
        ...
        source_dir_x86_android = oshelpers.fixpath(git_root, constants.build_root, "Android", "x86", "lib", "relwithdebinfo")
        ...
        source_dir_x86_android = oshelpers.fixpath(artifacts_path, "android_x86")
    ...
    target_path_x86_android = oshelpers.fixpath(git_root, plugin_path_under_project, "Android", "libs", "x86")
    ...
    copy_spatializer("Windows", source_dir_x64_desktop, target_path_x64_desktop)
    copy_spatializer("Windows", source_dir_x86_desktop, target_path_x86_desktop)
    copy_spatializer("WindowsStore", source_dir_x64_uwp, target_path_x64_uwp)
    copy_spatializer("WindowsStore", source_dir_x86_uwp, target_path_x86_uwp)
    copy_spatializer("WindowsStore", source_dir_ARM_uwp, target_path_ARM_uwp)
    copy_spatializer("WindowsStore", source_dir_ARM64_uwp, target_path_ARM64_uwp)
    copy_spatializer("Android", source_dir_x86_android, target_path_x86_android)
    copy_spatializer("Android", source_dir_ARM_android, target_path_ARM_android)
    copy_spatializer("Android", source_dir_ARM64_android, target_path_ARM64_android)
    ...
    copy_external("HrtfDsp", "Android", "x86", target_path_x86_android)
